...
function ChartTooltipContent({
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload
    const payloadItem = item as { dataKey?: string; name?: string }
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item: unknown, index: number) => {
          const it = item as { name?: string; dataKey?: string; payload?: { fill?: string }; color?: string; value?: unknown }
          ...
          return (
            ...
            >
              {formatter && it?.value !== undefined && it.name ? (
                formatter(it.value, it.name, item, index, payload)
              ) : (
                <>
                  {itemConfig?.icon ? (
                    <itemConfig.icon />
  ...
  return (
    ...
    >
      {payload.map((item: unknown) => {
        const it = item as { dataKey?: string; value?: unknown; color?: string }
        const key = `${nameKey || it.dataKey || 'value'}`
        const itemConfig = getPayloadConfigFromPayload(config, item, key)

        return (
          <div
            key={String(it.value ?? it.dataKey ?? key)}
            className={
